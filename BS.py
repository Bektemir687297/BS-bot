...
async def process_position(message: Message, state: FSMContext):
    ...
    try:
        cursor.execute(
            "INSERT OR REPLACE INTO users (user_id, full_name, work_place, position, approved) VALUES (?, ?, ?, ?, 0)",
...
@dp.message(lambda message: message.from_user.id == ADMIN_ID and message.photo,
            AddLocationState.waiting_for_first_photo)
...
@dp.message(lambda message: message.from_user.id == ADMIN_ID and message.photo,
            AddLocationState.waiting_for_second_photo)
async def process_second_photo(message: Message, state: FSMContext):
    ...
    await message.reply(
        "📸 Ikkita rasm qabul qilindi. Qo'shimcha ma'lumot yuboring (yo'q deb yozing agar kerak bo'lmasa):",
        protect_content=True)
...
async def add_location(message: Message, state: FSMContext):
    ...
    if current_state != AddLocationState.waiting_for_command.state:
        await message.reply("❌ Oldin ikkita rasm va ma'lumot yuboring! /reset_add bilan qayta boshlang.",
                            protect_content=True)
        await state.set_state(AddLocationState.waiting_for_first_photo)
        return
    match = re.match(r"/add\s+\[(\d+)\s+(.+?)\]\s+(http://maps.google.com/maps\?q=(-?\d+\.\d+),(-?\d+\.\d+)[^\s]*)",
                     message.text)
    if not match:
        return await message.reply("❌ Format: /add [3700 Aktash] http://maps.google.com/maps?q=39.919719,65.929442",
                                   protect_content=True)
    ...
    if not photo1 or not photo2:
        ...
        return
    cursor.execute(
        "INSERT OR REPLACE INTO locations (code, name, latitude, longitude, photo1, photo2, additional_info) "
...
async def get_location(message: Message, state: FSMContext):
    ...
    if not result or result[0] == 0:
        return await message.reply("❌ Admin ruxsatini olmagansiz. ⏳ Tasdiqni kuting.", reply_markup=get_user_keyboard(),
                                   protect_content=True)
    code = message.text.strip()
    if not code.isdigit():
        return await message.reply("❌ Kod faqat raqamlardan iborat bo‘lishi kerak (masalan, 3700)!",
                                   reply_markup=get_user_keyboard(), protect_content=True)
    cursor.execute("SELECT name, latitude, longitude, photo1, photo2, additional_info FROM locations WHERE code = ?",
                   (code,))
    result = cursor.fetchone()
    if not result:
        return await message.reply("❌ Kod topilmadi! Admin bilan bog‘laning.", reply_markup=get_user_keyboard(),
                                   protect_content=True)
    ...
    # Foydalanuvchi haqida ma'lumotni adminning lichkasiga yuborish
    cursor.execute("SELECT full_name FROM users WHERE user_id = ?", (user_id,))
    user_info = cursor.fetchone()
    full_name = user_info[0] if user_info else "Noma'lum"
    await bot.send_message(ADMIN_ID, f"📍 Foydalanuvchi {full_name} (ID: {user_id}) {code} kodli lokatsiyani oldi.",
                           protect_content=True)

    cursor.execute("INSERT INTO db_comments (user_id, comment) VALUES (?, ?)",
                   (user_id, f"{code} kodli lokatsiyani oldi"))
    conn.commit()
    await message.reply("📍 Yuqoridagi lokatsiya yuborildi. Tanlang:", reply_markup=get_location_action_keyboard(),
                        protect_content=True)
...
async def process_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    result = cursor.fetchone()
    if not result or result[0] == 0:
        await callback.message.reply("❌ Admin ruxsatini olmagansiz!", reply_markup=get_user_keyboard(),
                                     protect_content=True)
        return await callback.answer()

    if callback.data == "write_comment":
        await callback.message.reply(f"📞 Kommentariya yozish uchun {ADMIN_USERNAME} ga yozing.",
                                     reply_markup=get_user_keyboard(), protect_content=True)

    elif callback.data == "help":
        await callback.message.edit_text("ℹ️ Lokatsiya kodini yuboring (masalan, 3700).",
                                         reply_markup=get_user_keyboard(), protect_content=True)

    elif callback.data == "contact":
        await callback.message.edit_text(f"📞 {ADMIN_USERNAME} ga yozing.", reply_markup=get_user_keyboard(),
                                         protect_content=True)
...
async def handle_user_photo(message: Message):
    await message.reply("❌ Faqat lokatsiya kodi yuborishingiz mumkin (masalan, 3700)!",
                        reply_markup=get_user_keyboard(), protect_content=True)
